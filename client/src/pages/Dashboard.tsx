...
export default function Dashboard() {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-500 font-mono">LOADING WORKSPACE...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-950 font-sans relative">
      ...
      {/* Header */}
      <header className="h-16 xs:h-18 sm:h-20 border-b-4 border-black bg-white flex items-center justify-between px-4 xs:px-6 md:px-8 z-20 relative gap-2 shrink-0">
        <div className="flex items-center gap-3 xs:gap-4 sm:gap-6 min-w-0 flex-1">
          <button
            onClick={() => navigate("/")}
            className="hover:text-orange-600 transition-colors flex-shrink-0 group"
          >
          ...
          <h1 className="text-sm xs:text-base sm:text-2xl font-black text-zinc-950 tracking-tighter uppercase truncate">
            Dashboard
          </h1>
        </div>
        <button
      ...
      <div className="relative z-10 container mx-auto px-4 xs:px-6 py-6 sm:py-10">
         ...
         <div className="mb-8 sm:mb-12 space-y-4 sm:space-y-0 sm:flex sm:items-center sm:justify-between sm:gap-6">
           ...
           <div className="flex-1 min-w-0">
             <div className="relative group">
               ...
               <input
                 ...
                 className="w-full pl-12 pr-4 py-3 sm:py-4 bg-zinc-100 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] text-zinc-950 placeholder:text-zinc-400 rounded-none font-mono text-sm focus:outline-none focus:border-orange-600 transition-all"
               />
             </div>
           </div>
           ...
           <button
             onClick={() => navigate("/")}
             className="w-full sm:w-auto px-6 py-3 sm:py-4 font-mono text-xs sm:text-sm font-black uppercase bg-zinc-100 text-zinc-950 hover:bg-zinc-200 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-[2px] active:translate-y-[2px] active:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-none transition-all flex items-center justify-center gap-3 shrink-0"
        ...
        <div className="grid grid-cols-1 xs:grid-cols-3 gap-6 mb-10 sm:mb-16">
          {[
            ...
            <div key={i} className="bg-zinc-50 border-4 border-black p-5 sm:p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden group">
              ...
              <p className={`text-2xl sm:text-4xl font-black relative z-10 ${stat.color || "text-zinc-950"}`}>{stat.value}</p>
            </div>
          ))}
        </div>
        ...
        {filteredApps && filteredApps.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filteredApps.map((app: GeneratedApp) => (
              <div key={app.id} className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[12px_12px_0px_0px_rgba(234,88,12,0.3)] transition-all flex flex-col h-full group">
                <div className="p-5 border-b-4 border-black bg-zinc-100 group-hover:bg-zinc-200 transition-colors shrink-0">
                  <h3 className="text-base sm:text-lg font-black text-zinc-950 mb-2 line-clamp-1 uppercase tracking-tight group-hover:text-orange-500">{app.title}</h3>
                ...
                <div className="p-5 flex-1 flex flex-col justify-between gap-6">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      className="px-3 py-2.5 text-[10px] sm:text-xs font-mono font-black uppercase bg-zinc-100 text-zinc-950 border-2 border-black hover:bg-zinc-200 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-all flex items-center justify-center gap-2"
                      title="Edit app"
                    >
                    ...
                    <button
                      ...
                      className="col-span-2 px-3 py-2.5 text-[10px] sm:text-xs font-mono font-black uppercase bg-zinc-100 text-red-500 border-2 border-black hover:bg-red-50 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                      title="Delete app"
                    >
          ...
          <div className="bg-zinc-50 border-4 border-black p-12 sm:p-20 text-center shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]">
            <Code className="w-16 h-16 text-orange-600/20 mx-auto mb-6" />
            <h3 className="text-xl sm:text-3xl font-black text-zinc-950 mb-4 uppercase tracking-tighter">No constructions found</h3>
            <p className="text-sm sm:text-lg text-zinc-600 mb-10 max-w-md mx-auto leading-relaxed">
              {searchQuery
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="bg-white border-4 border-black text-zinc-950 rounded-none shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black uppercase tracking-tighter text-zinc-950">Confirm Destruction</DialogTitle>
          ...
          <DialogFooter className="gap-3 sm:gap-4 mt-6">
            <button
              ...
              disabled={deleteMutation.isPending}
              className="flex-1 px-4 py-3 text-xs font-mono font-black uppercase bg-zinc-100 text-zinc-950 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none"
            >
              Cancel
            </button>
            <button
